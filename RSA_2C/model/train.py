...
from model.utils import trim_padding_and_flat, concat_target_and_output, batch_auc, get_loss, get_targetPred, batch_acc, batch_pcc
...
def test(model, test_loader, criterion, device):
    ...
    with torch.no_grad():
        ...
        test_pcc = batch_pcc(all_target, all_pred)# * data.size(0)
    test_loss /= len(test_loader)
    # test_auc /= len(test_loader.dataset)
    print(test_pcc)
    print('\nTest set: Average loss: {:.4f}, AUC: {:.4f}, Accuracy: {:.4f}, PCC: {:.4f}\n'.format(test_loss, test_auc, test_acc, test_pcc))
    return test_loss, test_auc, test_acc, test_pcc
